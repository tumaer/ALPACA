...
/**
 * @brief Computes the trace of a dim x dim tensor for a full buffer at a
 * certain position
 ...
 */
constexpr double
ComputeTrace(double const (&tensor)[CC::TCX()][CC::TCY()][CC::TCZ()]
...
/**
 * @brief Computes the second Invariant of a tensor for a full buffer at a given
 * position
 * @param tensor Tensor for which the second invariant should be computed
 * @param tensor_squared squared of the tesnor for which the second invariant
 * should be computed
 * @return Second invarient
 */
constexpr double ComputeSecondInvariant(
    double const (&tensor)[CC::TCX()][CC::TCY()][CC::TCZ()][DTI(CC::DIM())]
                          [DTI(CC::DIM())],
    double const (&tensor_squared)[CC::TCX()][CC::TCY()][CC::TCZ()]
                                  [DTI(CC::DIM())][DTI(CC::DIM())],
    unsigned int const i, unsigned int const j, unsigned int const k) {
 ...
 */
constexpr double
ComputeTrace(std::array<std::array<double, 3>, 3> const &tensor) {
 ...
 * @param tensor Tensor for which the second invariant should be computed
 * @param tensor_squared Squared of the tensor for which the second invariant
 * should be computed
 * @return Second invarient of tensor
 */
constexpr double ComputeSecondInvariant(
    std::array<std::array<double, 3>, 3> const &tensor,
    std::array<std::array<double, 3>, 3> const &tensor_squared) {
 ...
 */
constexpr void ComputeSquaredOfTensor(
    double const (&tensor)[CC::TCX()][CC::TCY()][CC::TCZ()][DTI(CC::DIM())]
                          [DTI(CC::DIM())],
    double (&tensor_squared)[CC::TCX()][CC::TCY()][CC::TCZ()][DTI(CC::DIM())]
                            [DTI(CC::DIM())]) {
          ...
          for (unsigned int c = 0; c < DTI(CC::DIM()); ++c) {
            ...
            std::array<double, 3> const elements = {
                ...
                CC::DIM() != Dimension::One
                    ? tensor[i][j][k][r][1] * tensor[i][j][k][1][c]
                    : 0.0,
                CC::DIM() == Dimension::Three
                    ? tensor[i][j][k][r][2] * tensor[i][j][k][2][c]
                    : 0.0};
 ...
 * @param tensor_squared tensor where the squared is stored into (inidrect
 * return)
 */
constexpr void
ComputeSquaredOfTensor(std::array<std::array<double, 3>, 3> const &tensor,
                       std::array<std::array<double, 3>, 3> &tensor_squared) {

  for (unsigned int r = 0; r < 3; ++r) {
    for (unsigned int c = 0; c < 3; ++c) {
      std::array<double, 3> const elements = {tensor[0][r] * tensor[c][0],
                                              tensor[1][r] * tensor[c][1],
                                              tensor[2][r] * tensor[c][2]};
