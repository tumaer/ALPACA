 ...
 * @param maximum_value The maximum value allowed in the time stamp vector
 * (includint the value).
 */
inline void RemoveTimeStamps(std::vector<double> &time_stamps,
                             double const maximum_value) {
  time_stamps.erase(time_stamps.begin(),
                    std::find_if_not(time_stamps.begin(), time_stamps.end(),
                                     [&maximum_value](double const timestamp) {
 ...
 * @param output_folder Name of the output folder where the data is written
 * into.
 ...
 * @param standard_output_timestamps Timestamps when output is desired for the
 * standard or debug output.
 * @param interface_output_timestamps Timestamps when output is desired for the
 * interface output.
 * @param restore_mode Restore mode (Off, soft, forced).
 * @param restore_filename Filename that is used for restoring the simulation if
 * mode is enabled.
 * @param restart_snapshot_timestampsTimestamps when restart files are desired.
 * @param restart_snapshot_interval Interval (in wall seconds) when a restart
...
InputOutputManager::InputOutputManager(
    ...
    : // Start initializer list
      unit_handler_(unit_handler), logger_(LogWriter::Instance()),
      output_writer_(output_writer), restart_manager_(restart_manager),
      ...
      restore_mode_(restore_mode), restore_filename_(restore_filename),
      ...
      symlink_latest_restart_name_(
          output_folder_name_ + RestartSubfolderName() + LatestSnapshotName()),
      wall_time_of_last_restart_file_(std::chrono::system_clock::now()) {
  // This Barrier is needed, otherwise we get inconsistent folder names across
  // the ranks.
  ...
  if (MpiUtilities::MyRankId() == 0) {
    ...
    if (!input_file.empty()) {
      ...
      std::ofstream output_stream(output_folder_name_ + "/" +
                                      FileUtilities::RemoveFilePath(input_file),
                                  std::ios::binary);
    ...
    // Initialize the time series files (must be called after the
    // SetupOutputFolder() to ensure the presence of folders) If the time output
    // times stamps vectors are empty no output is desired
    std::string time_series_filename;
    if (!standard_output_timestamps_.empty()) {
      time_series_filename =
          OutputFileName(OutputType::Standard) + TimeSeriesSuffix();
      output_writer_.InitializeTimeSeriesFile(time_series_filename);
    }
    if (!interface_output_timestamps_.empty()) {
      time_series_filename =
          OutputFileName(OutputType::Interface) + TimeSeriesSuffix();
      output_writer_.InitializeTimeSeriesFile(time_series_filename);
    }
    if constexpr (DP::DebugOutput()) {
      time_series_filename =
          OutputFileName(OutputType::Debug) + TimeSeriesSuffix();
      output_writer_.InitializeTimeSeriesFile(time_series_filename);
    }
  }
}
 ...
 * @note In the destructor all operations are carried out to finalize the output
...
void InputOutputManager::CreateOutputFolder() const {
  ...
  if (standard_output_enabled_) {
    FileUtilities::CreateFolder(output_folder_name_ +
                                OutputSubfolderName(OutputType::Standard));
  }
  if (interface_output_enabled_) {
    FileUtilities::CreateFolder(output_folder_name_ +
                                OutputSubfolderName(OutputType::Interface));
  }
  if constexpr (DP::DebugOutput()) {
    FileUtilities::CreateFolder(output_folder_name_ +
                                OutputSubfolderName(OutputType::Debug));
  }
  ...
  std::ofstream output_stream(output_folder_name_ + "/.gitignore",
                              std::ios::out);
  output_stream << "*";
  output_stream.flush();
  output_stream.close();
...
/**
 * @brief Writes all micro timestep sizes performed during the last macro
 * timestep.
 * @param timesteps_on_finest_level Sizes of the micro timesteps.
 */
void InputOutputManager::WriteTimestepFile(
    std::vector<double> const &timesteps_on_finest_level) const {
  ...
  if (MpiUtilities::MyRankId() == 0) {
    ...
    // append to file
    FileUtilities::AppendToTextBasedFile(
        output_folder_name_ + "/micro_timesteps.txt", timesteps);
  }
}

/**
 * @brief Writes the full output (all outputs desired (standard, interface,
 * debug)) at the current timestep. If the force_output flag is set, output is
 * written in any case.
 ...
 */
bool InputOutputManager::WriteFullOutput(double const timestep,
                                         bool const force_output) {

  // Flag to indicate that any output has been written
  bool output_written = false;

  // dimensionalize time
  double const dimensionalized_time =
      unit_handler_.DimensionalizeValue(timestep, UnitType::Time);

  // string that is added to all filenames including the time
  std::string const time_name(
      std::to_string(dimensionalized_time * time_naming_factor_));

  // Add an empty line to the logger
  logger_.LogMessage(" ");

  // Check whether if the standard output is enabled (required to prevent call
  // .front() with empty vector )
  if (standard_output_enabled_) {
    // Check wether the output is forced or the next desired standard time stamp
    // is smaller than the current
    if (force_output || standard_output_timestamps_.front() <= timestep) {
      // erase the timestamps that are handled now (everything smaller than
      // current time step) This needs to be done since the macro timestep can
      // jump over several given timestamps
      RemoveTimeStamps(standard_output_timestamps_, timestep);

      // Write output with logging information for standard and debug output
      std::string const output_filename(OutputFileName(OutputType::Standard) +
                                        time_name);
      std::string const time_series_filename(
          OutputFileName(OutputType::Standard) + TimeSeriesSuffix());
      WriteOutput(OutputType::Standard, dimensionalized_time, output_filename,
                  time_series_filename);
  ...
  // Check whether if the interface output is enabled (required to prevent call
  // .front() with empty vector )
  if (interface_output_enabled_) {
    // Check wether the output is forced or the next desired interface time
    // stamp is smaller than the current
    if (force_output || interface_output_timestamps_.front() <= timestep) {
      ...
      std::string const output_filename(OutputFileName(OutputType::Interface) +
                                        time_name);
      std::string const time_series_filename(
          OutputFileName(OutputType::Interface) + TimeSeriesSuffix());
      WriteOutput(OutputType::Interface, dimensionalized_time, output_filename,
                  time_series_filename);
  ...
  if constexpr (DP::DebugOutput()) {
    std::string const output_filename(OutputFileName(OutputType::Debug) +
                                      time_name);
    std::string const time_series_filename(OutputFileName(OutputType::Debug) +
                                           TimeSeriesSuffix());
    WriteOutput(OutputType::Debug, dimensionalized_time, output_filename,
                time_series_filename);
...
/**
 * @brief Writes debug output with the given debug key (for detailed debug
 * output after algorithm substep).
 * @param output_key The output key indicating at which algorithm substep the
 * output is triggered.
 * @param output_type Output type identifier that is used (standard, interface,
 * debug). Default: Debug.
 */
void InputOutputManager::WriteSingleOutput(double const output_key,
...
/**
 * @brief Carries out the actual output writing (call of output writer) with
 * additional logging.
 * @param output_type Output type identifier that is used (standard, interface,
 * debug).
 * @param output_time Time of the output.
 * @param filename_without_extension Filename without extension where the output
 * is written into.
 * @param time_series_filename_without_extension Filename of the time series
 * data that is written.
 */
void InputOutputManager::WriteOutput(
  ...
  output_writer_.WriteOutput(output_type, output_time,
                             filename_without_extension,
                             time_series_filename_without_extension);

  // Final logging
  logger_.LogMessage(
      OutputTypeToString(output_type) + " output file written at t = " +
      StringOperations::ToScientificNotationString(output_time, 9));
  ...
  if (DP::Profile()) {
    ...
    MPI_Allgather(&seconds_elapsed, 1, MPI_DOUBLE, all_times.data(), 1,
                  MPI_DOUBLE, MPI_COMM_WORLD);
    std::vector<double>::iterator max_time =
    ...
    double const avg_time =
        std::accumulate(all_times.begin(), all_times.end(), 0.0) /
        number_of_ranks;
    std::string message = "Writing " + OutputTypeToString(output_type) +
                          " output: avg/max(rank)/min(rank) ";
    message.append(std::to_string(avg_time) + "/");
    message.append(std::to_string(*max_time) + "(" +
                   std::to_string(distance(all_times.begin(), max_time)) +
                   ")/");
    message.append(std::to_string(*min_time) + "(" +
                   std::to_string(distance(all_times.begin(), min_time)) +
                   ")/");
...
/**
 * @brief Writes a restart snapshot if necessary at the current timestep. Old
 * snapshots are deleted depending on user configuration.
 * @param force_output A flag indicating whether restart should be forced.
 * @param timestep The current timestep.
 * @param force_output A flag indicating whether output should be forced.
 */
void InputOutputManager::WriteRestartFile(double const timestep,
                                          bool const force_output) {
  ...
  if (restart_snapshot_interval_ > 0) {
    ...
    if (MpiUtilities::MyRankId() == 0) {
      std::chrono::time_point<std::chrono::system_clock> const
          current_wall_time = std::chrono::system_clock::now();
      int const wall_seconds_since_snapshot =
          std::chrono::duration_cast<std::chrono::seconds>(
              current_wall_time - wall_time_of_last_restart_file_)
              .count();
  ...
  // Check restart trigger on snapshot time stamps (first check on empty vector
  // and than compare to time step. Required to prevent call of .front() on
  // empty vector)
  bool snapshot_timestamp_triggered = !restart_snapshot_timestamps_.empty();
  if (snapshot_timestamp_triggered)
    snapshot_timestamp_triggered =
        restart_snapshot_timestamps_.front() <= timestep;

  // carry out restart writing if the restart is triggered by forcing, flagging
  // or timing
  if (snapshot_interval_triggered || snapshot_timestamp_triggered ||
      force_output) {
    ...
    std::string const time_name(std::to_string(
        unit_handler_.DimensionalizeValue(timestep, UnitType::Time) *
        time_naming_factor_));
    ...
    std::string snapshot_filename =
        restart_manager_.WriteRestartFile(timestep, filename_without_extension);
    ...
    if (MpiUtilities::MyRankId() == 0) {
      ...
      [[maybe_unused]] int const result_io =
          symlink(FileUtilities::RemoveFilePath(snapshot_filename).c_str(),
                  symlink_latest_restart_name_.c_str());
...
/**
 * @brief Restores the simulation from the user configured snapshot and returns
 * the simulation time of the snapshot.
 * @return The time of the simulation snapshot (negative if no restart is
 * carried out).
 */
double InputOutputManager::RestoreSimulationFromSnapshot() {
  ...
  case RestoreMode::Soft: {
    ...
    if (CheckIfRestoreFileExists()) {
      logger_.LogMessage("Initializing simulation from restart file: " +
                         restore_filename_);
      restart_time = restart_manager_.RestoreSimulation(restore_filename_);
    } else {
      logger_.LogMessage(
          "WARNING: restart file specified in the input file does not exist!");
      restart_time = -1.0;
    }
  } break;
  case RestoreMode::Forced: {
    if (!CheckIfRestoreFileExists()) {
      throw std::runtime_error("Force restore not possible: restart file "
                               "specified in the input file does not exist");
    }
  ...
  default:
    // as this method is only called once at startup, we don't need to make use
    // of the PERFORMANCE flag
  ...
  // Delete all timestamps smaller than restart time (if it is negative it will
  // have no affect since negative time stamps are already eliminated)
 ...
 * @param input_file The input currently processed. Indicates the path of the
 * output folder to be created.
 * @note Only the master ranks creates the folder - the resulting path is then
 * broadcasted to all ranks.
 */
std::filesystem::path
CreateOutputBaseFolder(std::filesystem::path const &input_file) {
  // due to MPI's lack of string-representation some bare metal code is needed
  // here ...
  std::uint64_t folder_path_size = 0;
  std::filesystem::path output_folder;
  if (MpiUtilities::MasterRank()) {
    output_folder =
        FileUtilities::AddUnusedNumberToPath(FileUtilities::RemoveFilePath(
  ...
  std::string output_folder_name = MpiUtilities::MasterRank()
                                       ? output_folder.string()
                                       : std::string(folder_path_size, '#');
  MPI_Bcast(output_folder_name.data(), folder_path_size, MPI_CHAR, 0,
            MPI_COMM_WORLD);
