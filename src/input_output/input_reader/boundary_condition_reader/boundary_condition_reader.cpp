 ...
 * @param location Boundary location for which the type is read (east, west, north, south, top, bottom).
 * @return Material boundary type.
 */
MaterialBoundaryType BoundaryConditionReader::ReadMaterialBoundaryType( BoundaryLocation const location ) const {
 ...
 */
LevelSetBoundaryType BoundaryConditionReader::ReadLevelsetBoundaryType( BoundaryLocation const location ) const {
   return StringToLevelSetBoundaryType( DoReadLevelSetBoundaryType( location ) );
}

/**
 * @brief Reads the (reduced) set of fixed value prime states at a given location.
 * @param location Boundary location for which the type is read (east, west, north, south, top, bottom).
...
std::array<double, MF::ANOP()> BoundaryConditionReader::ReadMaterialFixedValueBoundaryConditions( BoundaryLocation const location ) const {
   ...
   for( PrimeState const prime : MF::ASOP() ) {
      ...
      if( prime_name.empty() ) {
         ...
         fixed_values[PTI( prime )] = DoReadMaterialFixedValueBoundaryCondition( location, prime_name );
      }
   }
