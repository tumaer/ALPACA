...
/**
 * @brief Initializes and sorts lists of send/receive pairs of leaves for each
 * rank.
 ...
 * @param topology The topology to get information about the MPI status of the
 * simulation.
 */
void InitializeSendRecvRankLists(
    std::vector<std::vector<std::pair<nid_t, nid_t>>> &send_list,
    std::vector<std::vector<std::pair<nid_t, nid_t>>> &recv_list,
    std::vector<nid_t> const &local_leaf_ids, Direction const direction,
...
/**
 * @brief Calculate two dimensional offset of topology node with regards to root
 * at specific tree depth.
...
GetNeighborBlockOffset(nid_t const neighbor_id, int const neighbor_level_diff,
  ...
  for (int l = 0; l < neighbor_level_diff; l++) {
    if (sibling_func_1(i_node_id))
      node_offset_1 += power_two;
    if (sibling_func_2(i_node_id))
      node_offset_2 += power_two;
 ...
 * @param edge_a Offset of point shifted into first dimension by one in one
 * hexahedron.
 * @param edge_b Offset of point shifted into second dimension by one in one
 * hexahedron.
 * @param corner Offset of corner point in one hexahedron.
 */
void LoadBoundaryBuffer(unsigned long long int const *cell_vertex_ids,
                        unsigned long long int *buffer,
                        unsigned int const id_jump_a,
                        unsigned int const id_jump_b, unsigned int const size_a,
                        unsigned int const size_b, int const center,
                        int const edge_a, int const edge_b, int const corner) {
  for (unsigned int a = 0; a < size_a; a++) {
    for (unsigned int b = 0; b < size_b; b++) {
      ...
      if (b == size_b - 1) {
        buffer[buffer_index + size_a + 1] =
            cell_vertex_ids[vertex_index + edge_b];
        if (a == size_a - 1) {
          buffer[buffer_index + size_a + 2] =
              cell_vertex_ids[vertex_index + corner];
        }
      }
    }
  }
 ...
 * @param send_list_id_leveldiff List of node IDs paired with the tree
 * leveldifference to node_id to send data to.
 * @param send_buffer Buffer containing data to send.
 * @param direction Axis defining direction of communication.
 * @param send_list_per_rank List of total communications of MPI rank to define
 * a MPI tag.
 ...
 */
void SendFromNode(
    nid_t const node_id, std::vector<nid_t> const &send_id_list,
  ...
  for (auto const &target_id : send_id_list) {
    ...
    std::pair<unsigned int, unsigned int> const node_offset =
        GetNeighborBlockOffset(target_level_diff > 0 ? target_id : node_id,
                               abs_level_diff, direction);
    if (buf_size_1 == 1) {
      unsigned int const resolution_difference =
          (1 << abs_level_diff) / cell_count_axis_1;
      if (resolution_difference > 1 &&
          node_offset.first % resolution_difference ==
              0) { // lower boundary aligns
        if (target_level_diff > 0) {
          local_send_buffer_offset += node_offset.first / resolution_difference;
        }
      } else if (resolution_difference > 1 &&
                 (node_offset.first + 1) % resolution_difference ==
                     0) { // high boundary aligns
        if (target_level_diff < 0 &&
            (node_offset.first + 1) !=
                resolution_difference * cell_count_axis_1) {
          continue;
        }
        if (target_level_diff > 0) {
          local_send_buffer_offset +=
              (node_offset.first + 1) / resolution_difference;
        } else {
    ...
    if (buf_size_2 == 1) {
      unsigned int const resolution_difference =
          (1 << abs_level_diff) / cell_count_axis_2;
      if (resolution_difference > 1 &&
          node_offset.second % resolution_difference ==
              0) { // lower boundary aligns
        if (target_level_diff > 0) {
          local_send_buffer_offset += node_offset.second /
                                      resolution_difference *
                                      (cell_count_axis_1 + 1);
        }
      } else if (resolution_difference > 1 &&
                 (node_offset.second + 1) % resolution_difference ==
                     0) { // high boundary aligns
        if (target_level_diff < 0 &&
            (node_offset.second + 1) !=
                resolution_difference * cell_count_axis_2) {
          continue;
        }
        if (target_level_diff > 0) {
          local_send_buffer_offset += (node_offset.second + 1) /
                                      resolution_difference *
                                      (cell_count_axis_1 + 1);
        } else {
          local_send_buffer_offset +=
              (cell_count_axis_1 + 1) * cell_count_axis_2;
        }
      } else {
        continue; // no boundary aligns -> skip send to target node
      }
    ...
    if (target_level_diff > 0) {
      ...
      local_send_buffer_offset +=
          (buf_size_1 - 1) * node_offset.first +
          (cell_count_axis_1 + 1) * (buf_size_2 - 1) *
              node_offset.second; // here buffersizes are used as a scaling of
                                  // node_offset
      MPI_Type_vector(buf_size_2, buf_size_1, cell_count_axis_1 + 1,
                      MPI_UNSIGNED_LONG_LONG, &mpi_block_type);
    } else {
      ...
      MPI_Type_vector(buf_size_1, 1, stride, MPI_UNSIGNED_LONG_LONG,
                      &mpi_stride_type);
    ...
    std::vector<std::pair<nid_t, nid_t>> const send_list_target_rank =
        send_list_per_rank[target_rank];
    // look up rank in send_list; use its position as tag
    int const tag = std::distance(send_list_target_rank.begin(),
                                  lower_bound(send_list_target_rank.begin(),
                                              send_list_target_rank.end(),
                                              target_source_id)) %
                    MpiUtilities::MpiTagUb();

    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);

    mpi_send_request_list.push_back(MPI_Request());
    MPI_Isend(send_buffer + local_send_buffer_offset, 1, mpi_block_type,
              target_rank, tag, MPI_COMM_WORLD, &mpi_send_request_list.back());
 ...
 * @param recv_list_id_leveldiff List of node IDs paired with the tree
 * leveldifference to node_id to receive data from.
 * @param boundary_buffer Buffer to write received data into.
 * @param direction Axis defining direction of communication.
 * @param recv_list_per_rank List of total communications of MPI rank to define
 * a MPI tag.
 * @param topology The topology to get information about the MPI status of the
 * simulation.
 */
void RecvToNode(
    nid_t const node_id, std::vector<nid_t> const &recv_id_list,
  ...
  for (auto const &source_id : recv_id_list) {
    ...
    std::pair<unsigned int, unsigned int> const node_offset =
        GetNeighborBlockOffset(source_level_diff > 0 ? source_id : node_id,
                               abs_level_diff, direction);
    // if buffer size is one there are whole neighbor nodes to be skipped
    if (buf_size_1 == 1) {
      unsigned int const resolution_difference =
    ...
    if (buf_size_2 == 1) { // same as for direction 1
      unsigned int const resolution_difference =
          (1 << abs_level_diff) / cell_count_axis_2;
      if (resolution_difference > 1 &&
          node_offset.second % resolution_difference == 0) {
        if (source_level_diff > 0) {
          buffer_insert_offset += node_offset.second / resolution_difference *
                                  (cell_count_axis_1 + 1);
        }
      } else if (resolution_difference > 1 &&
                 (node_offset.second + 1) % resolution_difference == 0) {
        if (source_level_diff > 0 &&
            (node_offset.second + 1) !=
                resolution_difference * cell_count_axis_2) {
          continue;
        ...
        if (source_level_diff > 0) {
          buffer_insert_offset += (node_offset.second + 1) /
                                  resolution_difference *
                                  (cell_count_axis_1 + 1);
        } else {
    ...
    if (source_level_diff > 0) {
      ...
      buffer_insert_offset +=
          node_offset.second * (buf_size_2 - 1) * (cell_count_axis_1 + 1) +
          node_offset.first * (buf_size_1 - 1);
      MPI_Type_vector(buf_size_2, buf_size_1, cell_count_axis_1 + 1,
                      MPI_UNSIGNED_LONG_LONG, &mpi_insert_type);
    } else {
      // case: node is smaller or same size as neighbor
      int const stride = 1 << abs_level_diff;
      MPI_Datatype mpi_stride_type;
    ...
    std::vector<std::pair<nid_t, nid_t>> const recv_list_source_rank =
        recv_list_per_rank[source_rank];
    std::pair<nid_t, nid_t> const target_source_id(node_id, source_id);
    // look up location in recv list for tag
    int const tag =
        distance(recv_list_source_rank.begin(),
                 lower_bound(recv_list_source_rank.begin(),
                             recv_list_source_rank.end(), target_source_id)) %
        MpiUtilities::MpiTagUb();
    // recv data
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Recv(boundary_buffer + buffer_insert_offset, 1, mpi_insert_type,
