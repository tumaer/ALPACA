 ...
 */
BaroclinicityOutput::BaroclinicityOutput(
 ...
 */
void BaroclinicityOutput::DoComputeCellData(
  ...
  double const dimensionalization_factor = unit_handler_.DimensionalizeValue(
      1.0, {}, {UnitType::Time, UnitType::Time});
  ...
  if (node.HasLevelset()) {
    ...
    for (unsigned int k = CC::FICZ(); k <= CC::LICZ(); ++k) {
      for (unsigned int j = CC::FICY(); j <= CC::LICY(); ++j) {
        for (unsigned int i = CC::FICX(); i <= CC::LICX(); ++i) {
          std::array<double, 3> const pressure_gradient =
              SU::GradientVector<DerivativeStencil>(real_pressure, i, j, k,
                                                    cell_size);
          std::array<double, 3> const density_gradient =
              SU::GradientVector<DerivativeStencil>(real_density, i, j, k,
                                                    cell_size);
          cell_data[cell_data_counter++] =
              VU::L2Norm(
                  VU::CrossProduct(density_gradient, pressure_gradient)) /
              (real_density[i][j][k] * real_density[i][j][k]) *
    ...
    double const(&real_pressure)[CC::TCX()][CC::TCY()][CC::TCZ()] =
        block.GetPrimeStateBuffer(PrimeState::Pressure);
    double const(&real_density)[CC::TCX()][CC::TCY()][CC::TCZ()] =
 ...
 */
void BaroclinicityOutput::DoComputeDebugCellData(
    Node const &node, std::vector<double> &cell_data,
    unsigned long long int &cell_data_counter,
    [[maybe_unused]] MaterialName const material) const {
