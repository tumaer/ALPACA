//===---------------- matrix_tensor_material_output.h ---------------------===//
//
//                                 ALPACA
//
...
/**
 * @brief Example class for a matrix/tensor output quantity that writes material
 * data to the output. Material data is defined as all data operating on
 * material blocks. The difference to interface outputs is that those mainly
 * operate on interface blocks. Furthermore, a special distinction is made in
 * the debug output mode, where for material quantities for each material all
 * nodes are written. If the node does not contain the material a default value
 * is written.
 *
 * To define a new matrix material quantity this file can be used to define the
...
public:
  ...
  MatrixTensorMaterialOutput &
  operator=(MatrixTensorMaterialOutput const &) = delete;
