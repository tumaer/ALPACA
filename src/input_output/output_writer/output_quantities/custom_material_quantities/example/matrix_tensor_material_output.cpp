//===------------- matrix_tensor_material_output.cpp ----------------------===//
//
//                                 ALPACA
//
...
/**
 * @brief constructor to create a matrix/tensor material output quantity.
 ...
 * 0 : 00, 1: 01, 2: 10, 3: 11).
 */
MatrixTensorMaterialOutput::MatrixTensorMaterialOutput(
    ...
    : OutputQuantity(unit_handler, material_manager, quantity_name,
                     output_flags, {DTI(CC::DIM()), DTI(CC::DIM())}) {
 ...
 */
void MatrixTensorMaterialOutput::DoComputeCellData(
        ...
        for (unsigned int i = CC::FICX(); i <= CC::LICX(); ++i) {
          ...
          for (unsigned int row = 0; row < dimensions_[0]; row++) {
            for (unsigned int col = 0; col < dimensions_[1]; col++) {
              ...
              /**
               * Option 2: No preparation has been done. Differ between
          ...
          // cell_data_counter += dimensions[0] * dimensions[1] * dimensions[1];
        }
      }
    }
  } else {
 ...
 */
void MatrixTensorMaterialOutput::DoComputeDebugCellData(
    Node const &node, std::vector<double> &cell_data,
