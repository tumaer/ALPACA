...
namespace Simulation {
   ...
   void Run( InputReader const& input_reader ) {
      ...
      logger.SetLogfile( output_folder / input_file.filename().replace_extension( ".log" ) );
      logger.LogBreakLine();
      logger.Flush();
      ...
      logger.LogMessage( "Number of MPI ranks : " + std::to_string( MpiUtilities::NumberOfRanks() ) );
      ...
      InputOutputManager input_output_manager( Instantiation::InstantiateInputOutputManager( input_reader, output_writer, restart_manager, unit_handler, output_folder ) );
      logger.LogBreakLine();
      logger.Flush();
