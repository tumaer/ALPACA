 ...
 */
double WENO5Z::ApplyImplementation( std::array<double, stencil_size_> const& array, std::array<int const, 2> const evaluation_properties, const double cell_size) const {
   ...
   if(array.size() < stencil_size_) {
      throw std::logic_error("Stencil size in WENO5-Z is longer than provided Array");
   }
#endif
   ...
   const double a2 = coef_weights_2_ * ( 1.0 + tau / (s2 + epsilon_) );
