...
class WENO7 : public Stencil<WENO7> {
  ...
  /**
   * @brief Evaluates the stencil according to a WENO-7 scheme. Also See base
   * class.
  ...
  ApplyImplementation(std::array<double, stencil_size_> const &array,
    ...
    //       s4 are of similar magnitude. Then, it cannot guaranteed anymore
    //       that a division by zero is avoided.
    double const a1 =
        coef_weights_1_ / ((s1 + epsilon_weno7_) * (s1 + epsilon_weno7_));
    double const a2 =
        coef_weights_2_ / ((s2 + epsilon_weno7_) * (s2 + epsilon_weno7_));
    double const a3 =
        coef_weights_3_ / ((s3 + epsilon_weno7_) * (s3 + epsilon_weno7_));
    double const a4 =
        coef_weights_4_ / ((s4 + epsilon_weno7_) * (s4 + epsilon_weno7_));
