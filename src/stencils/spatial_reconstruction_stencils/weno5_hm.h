 ...
 * fluxes according to \cite Zeng2019
 */
class WENO5HM : public Stencil<WENO5HM> {
  ...
  ApplyImplementation(std::array<double, stencil_size_> const &array,
    ...
    // New multi-step function that gives higher-order accuracy in transition
    // region
    double const h1 = 2.0 * s1 + ((s1 - s2) / (s1 + s2 + epsilon_)) *
                                     ((s1 - s2) / (s1 + s2 + epsilon_)) * s2;
    double const h3 = 2.0 * s3 + ((s3 - s2) / (s3 + s2 + epsilon_)) *
                                     ((s3 - s2) / (s3 + s2 + epsilon_)) * s2;
    ...
    double const a2 =
        coef_weights_2_ * (1.0 + tau / (h1 + epsilon_) + tau / (h3 + epsilon_));
...
public:
  explicit constexpr WENO5HM() = default;
  ~WENO5HM() = default;
