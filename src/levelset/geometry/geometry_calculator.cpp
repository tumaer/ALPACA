         ...
         for( unsigned int k = CC::FICZ(); k <= CC::LICZ(); ++k ) {
            if( std::abs( interface_tags[i][j][k] ) <= ITTI( IT::NewCutCell ) ) {
               ...
               double const first_summand = ConsistencyManagedSum(
                     levelset_x[i][j][k] * levelset_x[i][j][k] * ( second_derivatives[1][1] + second_derivatives[2][2] ), levelset_y[i][j][k] * levelset_y[i][j][k] * ( second_derivatives[0][0] + second_derivatives[2][2] ), levelset_z[i][j][k] * levelset_z[i][j][k] * ( second_derivatives[0][0] + second_derivatives[1][1] ) );

               double const second_summand = -2.0 * ConsistencyManagedSum(
                                                          ( levelset_x[i][j][k] * levelset_y[i][j][k] ) * second_derivatives[0][1], ( levelset_x[i][j][k] * levelset_z[i][j][k] ) * second_derivatives[0][2], ( levelset_y[i][j][k] * levelset_z[i][j][k] ) * second_derivatives[1][2] );
