//===------------- fedkiw_iterative_ghost_fluid_extender.h ----------------===//
//
//                                 ALPACA
//
...
//
//===----------------------------------------------------------------------===//
#ifndef FEDKIW_GHOST_FLUID_EXTENDER_H
#define FEDKIW_GHOST_FLUID_EXTENDER_H

#include "ghost_fluid_extender.h"
#include "utilities/mathematical_functions.h"

/**
 * @brief The FedkiwGhostFluidExtender class extends material-states from
 * real-material to ghost-material by iteratively solving the extension
 * equation. The approach follows \cite osher2006level.
 *
...
class FedkiwGhostFluidExtender
    : public GhostFluidExtender<FedkiwGhostFluidExtender<field_type>,
                                field_type> {
  // Definition for shortening
  using GhostFluidExtenderSpecification =
      GhostFluidExtender<FedkiwGhostFluidExtender<field_type>, field_type>;
  // friend declaration
        ...
        for (unsigned int i = CC::FICX() - i_offset_;
          ...
          for (unsigned int j = CC::FICY() - j_offset_;
            ...
            for (unsigned int k = CC::FICZ() - k_offset_;
              ...
              /**
               * JW: We also extend in the reinitialization band in order to
               * have better convergence behaviour (Reduce influence of
               * implicitly imposed boundary conditions at the end of the narrow
               * band). The convergence criteria is only checked for the
               * extension band.
               */
              if (std::abs(interface_tags[i][j][k]) <=
                ...
                if (-material_sign * levelset[i + 1][j][k] >
                        -material_sign * levelset[i][j][k] &&
                    -material_sign * levelset[i - 1][j][k] >
                        -material_sign * levelset[i][j][k]) {
                ...
                if constexpr (CC::DIM() != Dimension::One) {
                  if (-material_sign * levelset[i][j + 1][k] >
                          -material_sign * levelset[i][j][k] &&
                      -material_sign * levelset[i][j - 1][k] >
                          -material_sign * levelset[i][j][k]) {
                ...
                if constexpr (CC::DIM() == Dimension::Three) {
                  if (-material_sign * levelset[i][j][k + 1] >
                          -material_sign * levelset[i][j][k] &&
                      -material_sign * levelset[i][j][k - 1] >
                          -material_sign * levelset[i][j][k]) {
...
public:
  ...
  explicit FedkiwGhostFluidExtender(MaterialManager const &material_manager,
  ...
  FedkiwGhostFluidExtender &
  operator=(FedkiwGhostFluidExtender const &) = delete;
