...
/**
 * @brief Class that implements the main functionality for mixing between two
 * fluids. Specializations necessary since different choices for the mixing
 * contributions exist.
 * @tparam DerivedTwoPhaseCutCellMixer Typename as template parameter due to
 * CRTP.
 */
...
private:
  ...
  /**
   * @brief FICMO Gives the index of the First Internal Cell Minus One in a
   * block per dimension.
   */
  static constexpr unsigned int FICMOX = CC::HS() - 1;
  static constexpr unsigned int FICMOY =
      CC::DIM() != Dimension::One ? CC::HS() - 1 : 0;
  static constexpr unsigned int FICMOZ =
      CC::DIM() == Dimension::Three ? CC::HS() - 1 : 0;

  /**
   * @brief LICPO Gives the index of the Last Internal Cell Plus One in a block
   * per dimension. I. e. the returned index must be included if the internal
   * cells are of interest.
   */
  static constexpr unsigned int LICPOX = CC::HS() + CC::ICX();
  static constexpr unsigned int LICPOY =
      CC::DIM() != Dimension::One ? (CC::HS() + CC::ICY()) : 0;
  static constexpr unsigned int LICPOZ =
      CC::DIM() == Dimension::Three ? (CC::HS() + CC::ICZ()) : 0;
   ...
   * methods.
   */
  explicit TwoPhaseCutCellMixer(HaloManager &halo_manager,
                                unsigned int const number_of_mixing_operations,
  ...
  /**
   * @brief Performs mixing on given nodes. Mixing weights are obtained by a
   * interface-normal based splitting.
   * @param node See base class.
   */
  void MixImplementation(Node &node) const {

    /***
     * mixing_contributions contains the relevant information about the mixing
     * operations of one block. The outermost vector separates the mixing
     * contributions cell-wise. The following pair is to separate unsigned-int
     * and double-valued information about a mixing operations. Each element of
     * the vector of a pair contains information about a single mixing
     * operation. First vector (unsigned int): i, j, k, i_target, j_target,
     * k_target. Second vector (double): mixing fraction, factor to calculate
     * mixing fluxes.
     */
    std::vector<std::pair<std::vector<std::array<unsigned int, 6>>,
                          std::vector<std::array<double, 2>>>>
        mixing_contributions;
    ...
    for (auto const &phase : node.GetPhases()) {
      ...
      static_cast<DerivedTwoPhaseCutCellMixer const &>(*this)
          .CalculateMixingContributionsImplementation(node, material,
                                                      mixing_contributions);
      for (unsigned int c = 0; c < number_of_mixing_operations_; ++c) {
        CalculateMixingFluxes(node, material, mixing_contributions, c,
                              mixing_fluxes);
      }
  ...
  /**
   * @brief Adds the mixing fluxes saved in conservative_change to the
   * right-hand side conservative buffer of a specific phase.
   * @param node The node which contains the phase that has to be mixed.
   * @param material The material which specifies the phase that has to be
   * mixed.
   * @param mixing_fluxes The mixing fluxes which are added to the right-hand
   * side buffer.
   */
  void AddMixingFluxesToConservatives(
      Node &node, MaterialName const material,
      double const (
          &mixing_fluxes)[MF::ANOE()][CC::TCX()][CC::TCY()][CC::TCZ()]) const {
    ...
    for (Equation const eq : MF::ASOE()) {
      double(&conservatives)[CC::TCX()][CC::TCY()][CC::TCZ()] =
          phase.GetRightHandSideBuffer(eq);
  ...
  /**
   * @brief Based on given mixing contributions, i.e. cell-pairs which are
   * mixed, the mixing fluxes are calculated.
   * @param node The node for which the mixing fluxes are calculated.
   * @param material The material which specifies the phase for which the mixing
   * contributions are calculated.
   * @param mixing_contributions The mixing contributions for which the
   * corresponding fluxes are calculated. A mixing contribution tuple contains
   * information about the mixing operation between two cells. It contains the
   * indices of the target cell (i_target, j_target and k_target - saved as
   * unsigned int), the mixing fraction beta (saved as double) and a factor
   * necessary to calculate the mixing fluxes (saved a a double).
   * @param contribution_identifier An identifier which mixing contribution is
   * considered.
   * @param mixing_fluxes Indirect return parameter for the mixing fluxes.
   */
  void CalculateMixingFluxes(
    ...
    for (auto const &mix_element : mixing_contributions) {
      std::vector<std::array<unsigned int, 6>> const &indices =
          mix_element.first;
      std::vector<std::array<double, 2>> const &flux_weights =
          mix_element.second;
      if (contribution_identifier < indices.size()) {
        ...
        double const volume_fraction_target =
            reference_volume_fraction +
            material_sign_double *
                volume_fraction[i_target][j_target][k_target];
        double const volume_fraction_self =
            reference_volume_fraction +
            material_sign_double * volume_fraction[i][j][k];

        for (Equation const eq : MF::ASOE()) {
          double const conservative_target =
              phase.GetRightHandSideBuffer(eq)[i_target][j_target][k_target];
          double const conservative_self =
              phase.GetRightHandSideBuffer(eq)[i][j][k];
          double const M =
              factor * (conservative_target * volume_fraction_self -
                        conservative_self * volume_fraction_target);
