...
/**
 * @brief Returns a pointer to the equation of state obtained from the given
 * input data.
 * @param eos_name Name of the equation of state.
 * @param eos_data All parameters specified for the equation of state from the
 * input file.
 ...
 */
std::unique_ptr<EquationOfState const> InstantiateEquationOfState(
    EquationOfStateName const eos_name,
  ...
  case EquationOfStateName::StiffenedGas: {
    ...
    std::unique_ptr<StiffenedGas const> eos(
        std::make_unique<StiffenedGas const>(eos_data, unit_handler));
    logger.LogMessage(eos->GetLogData(4, unit_handler));
    return eos;
  }
  case EquationOfStateName::StiffenedGasSafe: {
    // 1. Create, 2. Log, 3. Return eos
    std::unique_ptr<StiffenedGasSafe const> eos(
        std::make_unique<StiffenedGasSafe const>(eos_data, unit_handler));
    logger.LogMessage(eos->GetLogData(4, unit_handler));
    return eos;
  }
  case EquationOfStateName::StiffenedGasCompleteSafe: {
    // 1. Create, 2. Log, 3. Return eos
    std::unique_ptr<StiffenedGasCompleteSafe const> eos(
        std::make_unique<StiffenedGasCompleteSafe const>(eos_data,
                                                         unit_handler));
    logger.LogMessage(eos->GetLogData(4, unit_handler));
    return eos;
  }
  case EquationOfStateName::WaterlikeFluid: {
    // 1. Create, 2. Log, 3. Return eos
    std::unique_ptr<WaterlikeFluid const> eos(
        std::make_unique<WaterlikeFluid const>(eos_data, unit_handler));
    logger.LogMessage(eos->GetLogData(4, unit_handler));
    return eos;
  }
  case EquationOfStateName::NobleAbelStiffenedGas: {
    ...
    if constexpr (!CC::GruneisenDensityDependent()) {
      throw std::runtime_error("To use NobleAbelStiffenedGas you need to "
                               "activate CC::GruneisenDensityDependent");
    }
    // 1. Create, 2. Log, 3. Return eos
    std::unique_ptr<NobleAbelStiffenedGas const> eos(
        std::make_unique<NobleAbelStiffenedGas const>(eos_data, unit_handler));
    logger.LogMessage(eos->GetLogData(4, unit_handler));
    return eos;
  }
  case EquationOfStateName::Isentropic: {
    // 1. Create, 2. Log, 3. Return eos
    std::unique_ptr<Isentropic const> eos(
        std::make_unique<Isentropic const>(eos_data, unit_handler));
    logger.LogMessage(eos->GetLogData(4, unit_handler));
    return eos;
  }
  default: {
...
/**
 * @brief Gives the model for the material property shear viscosity for the
 * given input data.
 ...
 */
std::unique_ptr<MaterialParameterModel const> InstantiateShearViscosityModel(
  ...
  case MaterialPropertyModelName::ShearViscosityConstant: {
    ...
    std::unique_ptr<ConstantShearViscosityModel const> model(
        std::make_unique<ConstantShearViscosityModel const>(model_data,
                                                            unit_handler));
  ...
  case MaterialPropertyModelName::ShearViscosityPowerLaw: {
    // 1. Create, 2. Log, 3. Return model
    std::unique_ptr<PowerLawShearViscosityModel const> model(
        std::make_unique<PowerLawShearViscosityModel const>(model_data,
                                                            unit_handler));
    logger.LogMessage(model->GetLogData(6, unit_handler));
    return model;
  }
  case MaterialPropertyModelName::ShearViscosityCarreauYasuda: {
    // 1. Create, 2. Log, 3. Return model
    std::unique_ptr<CarreauYasudaShearViscosityModel const> model(
        std::make_unique<CarreauYasudaShearViscosityModel const>(model_data,
                                                                 unit_handler));
    logger.LogMessage(model->GetLogData(6, unit_handler));
    return model;
  }
  case MaterialPropertyModelName::ShearViscosityCross: {
    // 1. Create, 2. Log, 3. Return model
    std::unique_ptr<CrossShearViscosityModel const> model(
        std::make_unique<CrossShearViscosityModel const>(model_data,
                                                         unit_handler));
    logger.LogMessage(model->GetLogData(6, unit_handler));
    return model;
  }
  case MaterialPropertyModelName::ShearViscositySutherland: {
    // 1. Create, 2. Log, 3. Return model
    std::unique_ptr<SutherlandShearViscosityModel const> model(
        std::make_unique<SutherlandShearViscosityModel const>(model_data,
                                                              unit_handler));
    logger.LogMessage(model->GetLogData(6, unit_handler));
    return model;
  }
  default: {
    throw std::logic_error(
        "This shear viscosity model is not implemented yet!");
  }
  }
}

/**
 * @brief Gives the model for the material property thermal conductivity for the
 * given input data.
 ...
 * @return pointer to the const base class of all material parameter models.
 */
std::unique_ptr<MaterialParameterModel const>
InstantiateThermalConductivityModel(
    MaterialPropertyModelName const model_name,
    std::unordered_map<std::string, double> const &model_data,
    UnitHandler const &unit_handler) {
  // logger
  ...
  case MaterialPropertyModelName::ThermalConductivityConstant: {
    // 1. Create, 2. Log, 3. Return model
    std::unique_ptr<ConstantThermalConductivityModel const> model(
        std::make_unique<ConstantThermalConductivityModel const>(model_data,
                                                                 unit_handler));
    logger.LogMessage(model->GetLogData(6, unit_handler));
    return model;
  }
  default: {
    throw std::logic_error(
        "This thermal conductivity model is not implemented yet!");
  }
  }
}
 ...
 */
MaterialType InstantiateMaterialType(unsigned int const material_index,
                                     MaterialReader const &material_reader) {
  // Read the material type (default to Fluid if not given)
  MaterialType const material_type(
      material_reader.ReadMaterialType(material_index, MaterialType::Fluid));

  // Make consistency checks, that the type corresponds to a type that is
  // allowed with current user specifications
  if constexpr (!CC::SolidBoundaryActive()) {
    if (material_type == MaterialType::SolidBoundary) {
      throw std::runtime_error(
          "To use solid boundary materials active the compile time constants.");
    }
  }

  return material_type;
}
/* @brief Instantiates a complete material with the given input reader.* @param
 * material_index Index of the material to be read from the input file.* @param
 * material_reader Reader that provides access to the material data of the input
 * file.* @param unit_handler Instance to provide( non - ) dimensionalization of
 * values.* @ return Full initialized material.*
 ...
 */
std::tuple<MaterialType, Material>
InstantiateMaterial(unsigned int const material_index,
                    MaterialReader const &material_reader,
                    UnitHandler const &unit_handler) {
  // Read all data from input file
  // Read the material type (default to Fluid if not given)
  MaterialType const material_type(
      InstantiateMaterialType(material_index + 1, material_reader));

  // Always read the data for the equation of state
  EquationOfStateName const eos_name(
      material_reader.ReadEquationOfStateName(material_index + 1));
  // data map cannot be const since specific heat is added below in case for
  // NobelAbelStiffened Gas
  std::unordered_map<std::string, double> eos_data(
      material_reader.ReadEquationOfStateData(material_index + 1));
  ...
  MaterialPropertyModelName shear_viscosity_model_name =
  ...
  MaterialPropertyModelName thermal_conductivity_model_name =
      MaterialPropertyModelName::NotUsed;
  ...
  // material index +1 is used since in input file the indices start at 1 and
  // internally at 0
  ...
  if constexpr (CC::ViscosityIsActive()) {
    ...
    bulk_viscosity_fixed_value = material_reader.ReadFixedValue(
        material_indices, MaterialProperty::BulkViscosity);
    // shear viscosity
    // read model data if required
    if constexpr (CC::ShearViscosityModelActive()) {
      // read the model data
      shear_viscosity_model_name = material_reader.ReadModelName(
          material_indices, MaterialProperty::ShearViscosity);
      shear_viscosity_model_data = material_reader.ReadModelData(
          material_indices, MaterialProperty::ShearViscosity);
    } else { // otherwise read the fixed value
      shear_viscosity_fixed_value = material_reader.ReadFixedValue(
          material_indices, MaterialProperty::ShearViscosity);
    }
  }
  ...
  if constexpr (CC::HeatConductionActive()) {
    ...
    if constexpr (CC::ThermalConductivityModelActive()) {
      // read the model data
      thermal_conductivity_model_name = material_reader.ReadModelName(
          material_indices, MaterialProperty::ThermalConductivity);
      thermal_conductivity_model_data = material_reader.ReadModelData(
          material_indices, MaterialProperty::ThermalConductivity);
    } else {
      // thermal conductivity fixed value
      thermal_conductivity_fixed_value = material_reader.ReadFixedValue(
          material_indices, MaterialProperty::ThermalConductivity);
    }
  }
  ...
  if constexpr (CC::HeatConductionActive()) {
    specific_heat_capacity_fixed_value = material_reader.ReadFixedValue(
        material_indices, MaterialProperty::SpecificHeatCapacity);
  }

  // specific heat (property for equation of state). Second read cycle to not
  // fill default specific heat fixed value.
  if (eos_name == EquationOfStateName::NobleAbelStiffenedGas) {
    std::string const specific_heat_name(MaterialPropertyToString(
        MaterialProperty::SpecificHeatCapacity, false));
    eos_data[specific_heat_name] = material_reader.ReadFixedValue(
        material_indices, MaterialProperty::SpecificHeatCapacity);
  }
  ...
  logger.LogMessage(StringOperations::Indent(2) +
                    "Material type: " + MaterialTypeToString(material_type));
  ...
  // create eos
  std::unique_ptr<EquationOfState const> eos(
      InstantiateEquationOfState(eos_name, eos_data, unit_handler));
  ...
  if constexpr (CC::ViscosityIsActive()) {
    logger.LogMessage(tmp_string + StringOperations::ToScientificNotationString(
                                       bulk_viscosity_fixed_value, 9));
  } else {
  ...
  if constexpr (CC::ViscosityIsActive()) {
    if (shear_viscosity_model_name != MaterialPropertyModelName::NotUsed) {
      ...
      shear_viscosity_model = InstantiateShearViscosityModel(
          shear_viscosity_model_name, shear_viscosity_model_data, unit_handler);
    } else {
      logger.LogMessage(tmp_string +
                        StringOperations::ToScientificNotationString(
                            shear_viscosity_fixed_value, 9));
    }
  } else {
    logger.LogMessage(tmp_string + "Not Used");
  }
  ...
  if constexpr (CC::HeatConductionActive()) {
    if (thermal_conductivity_model_name != MaterialPropertyModelName::NotUsed) {
      // create model (logs data itself)
      logger.LogMessage(tmp_string);
      thermal_conductivity_model = InstantiateThermalConductivityModel(
          thermal_conductivity_model_name, thermal_conductivity_model_data,
          unit_handler);
    } else {
      logger.LogMessage(tmp_string +
                        StringOperations::ToScientificNotationString(
                            thermal_conductivity_fixed_value, 9));
    }
  } else {
    logger.LogMessage(tmp_string + "Not Used");
  }
  ...
  if constexpr (CC::HeatConductionActive()) {
    logger.LogMessage(tmp_string + StringOperations::ToScientificNotationString(
                                       specific_heat_capacity_fixed_value, 9));
  } else {
    logger.LogMessage(tmp_string + "Not Used");
  }
  ...
  // Return the final fully instantiated material ( move operations to transfer
  // pointer ownership (deleted move constructor))
  return std::make_tuple(
      material_type,
      Material(std::move(eos), bulk_viscosity_fixed_value,
               shear_viscosity_fixed_value, thermal_conductivity_fixed_value,
               specific_heat_capacity_fixed_value,
               std::move(shear_viscosity_model),
               std::move(thermal_conductivity_model), unit_handler));
}
} // namespace Instantiation
