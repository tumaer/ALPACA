//===------------- instantiation_initial_condition.cpp --------------------===//
//
//                                 ALPACA
//
...
/**
 * @brief Reads the initial material condition expression string for all
 * materials.
 * @param initial_condition_reader Instance that provides access to the initial
 * condition data in the input file.
 * @param number_of_materials Number of materials present in the current
 * simulation.
 * @param prime_state_variable_names All names of the prime states that are read
 * (contains empty strings for prime states that should not be read).
...
std::vector<std::string> GetMaterialInitialConditions(
  ...
  for (unsigned int material_index = 0; material_index < number_of_materials;
       material_index++) {
    // material index +1 is used since in input file the indices start at 1 and
    // internally at 0
    material_initial_conditions[material_index] =
        initial_condition_reader.ReadMaterialInitialConditions(material_index +
                                                               1);
    // Check if all non-empty prime state variables are given. Otherwise throw a
    // warning
    for (auto const &variable_name : prime_state_variable_names) {
      if (!variable_name.empty()) {
        if (material_initial_conditions[material_index].find(variable_name) ==
            std::string::npos) {
          logger.LogMessage(" ");
          logger.LogMessage("Warning!! The variable name '" + variable_name +
                            "' is not contained in the initial condition "
                            "string for material " +
 ...
 * @param levelset_index The levelset index for which the variables should be
 * read.
...
std::array<ParametricVariable, 2> CreateParametricVariables(
  ...
  // Read all variables
  std::vector<ParametricVariable> const parametric_variables(
      initial_condition_reader.ReadParametricLevelsetInitializerVariables(
          levelset_index));
  // Assign the correct number of variables depending on the given input an used
  // dimensions
...
/**
 * @brief Returns a pointer to the levelset initializer obtained from the given
 * input data.
 * @param initial_condition_reader Instance to read the initial condition data.
 * @param levelset_index The levelset index for which the variables should be
 * read.
...
std::unique_ptr<LevelsetInitializer> InstantiateLevelsetInitializer(
  ...
  // and assign a constant expression (cannot be empty. UserExpression throws
  // error otherwise.)
  if (material_names.size() == 1) {
    return std::make_unique<FunctionalLevelsetInitializer>(
        "phi := 1.0;", std::vector<std::array<double, 6>>(), material_names,
        node_size_on_level_zero_, maximum_level);
  }
  // read the type and input of the levelset initializer
  LevelsetInitializerType const initializer_type{
      initial_condition_reader.ReadLevelsetInitializerType(
          levelset_index, LevelsetInitializerType::Functional)};
  std::string const initializer_input{
      initial_condition_reader.ReadLevelsetInitializerInput(levelset_index)};
  // read the bounding boxex
  std::vector<std::array<double, 6>> bounding_boxes{
      initial_condition_reader.ReadLevelsetInitializerBoundingBoxes(
          levelset_index)};
  ...
  // switch between different levelset initializer types to call specific
  // constructor
  ...
  case LevelsetInitializerType::Functional: {
    ...
    std::unique_ptr<LevelsetInitializer> levelset_initializer(
        std::make_unique<FunctionalLevelsetInitializer>(
            initializer_input, bounding_boxes, material_names,
            node_size_on_level_zero_, maximum_level));
  ...
  case LevelsetInitializerType::STL: {
    // 1. Create, 2. Log, 3. Return levelset_initializer
    std::unique_ptr<LevelsetInitializer> levelset_initializer(
        std::make_unique<StlLevelsetInitializer>(
            StringOperations::RemoveSpaces(initializer_input), bounding_boxes,
            material_names, node_size_on_level_zero_, maximum_level));
    logger.LogMessage(levelset_initializer->GetLogData(2));
    logger.LogMessage(" ");
    return levelset_initializer;
  }
  case LevelsetInitializerType::Parametric: {
    ...
    std::array<ParametricVariable, 2> const parametric_variables(
        CreateParametricVariables(initial_condition_reader, levelset_index));
    std::array<double, 3> const reference_point(
        initial_condition_reader
            .ReadParametricLevelsetInitializerReferencePoint(levelset_index));
    // 1. Create, 2. Log, 3. Return levelset_initializer
    std::unique_ptr<LevelsetInitializer> levelset_initializer(
        std::make_unique<ParametricLevelsetInitializer>(
            initializer_input, parametric_variables, reference_point,
            bounding_boxes, material_names, node_size_on_level_zero_,
            maximum_level));
    logger.LogMessage(levelset_initializer->GetLogData(2));
    logger.LogMessage(" ");
    return levelset_initializer;
  ...
  default: {
    throw std::logic_error(
        "The chosen levelset initialization method has not been implemented!");
  }
  }
}

/**
 * @brief Instantiates the complete initial condition class with the given input
 * classes.
...
std::unique_ptr<InitialCondition> InstantiateInitialCondition(
  ...
  // Get all variables that used in the initial condition string (defined in
  // field details)
  std::vector<std::string> prime_state_variable_names(MF::ANOP());
  for (PrimeState const p : MF::ASOP()) {
    // No check is done here if variable should be used for input. Later 0 is
    // assigned to those
    prime_state_variable_names[PTI(p)] =
        StringOperations::RemoveSpaces(std::string(MF::InputName(p)));
  }

  // For more than one initializer this must be a loop and the resulting
  // initializer a vector
  unsigned int const levelset_index = 0;

  std::unique_ptr<LevelsetInitializer> levelset_initializer(
      InstantiateLevelsetInitializer(
          input_reader.GetInitialConditionReader(), levelset_index + 1,
          material_manager.GetMaterialNames(),
          unit_handler.DimensionalizeValue(tree.GetNodeSizeOnLevelZero(),
                                           UnitType::Length),
          topology_manager.GetMaximumLevel()));

  // Initialize the prime state handler
  std::vector<std::string> const material_initial_conditions{
      GetMaterialInitialConditions(input_reader.GetInitialConditionReader(),
  ...
  std::unique_ptr<PrimeStateInitializer const> prime_state_initializer{
      std::make_unique<PrimeStateInitializer const>(
          material_initial_conditions, prime_state_variable_names,
          unit_handler.DimensionalizeValue(tree.GetNodeSizeOnLevelZero(),
                                           UnitType::Length),
          unit_handler)};
