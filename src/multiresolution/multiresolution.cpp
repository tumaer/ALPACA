 ...
 * @param thresholder Thresholder used to compute the maximum allowed details in
 * the multiresolution analysis.
 */
Multiresolution::Multiresolution(Thresholder &&thresholder)
 ...
 * @param child_id The id of the child whose values are averaged into the
 * parent.
 * @note Adds the average to the already present value in the parent buffer.
 */
void Multiresolution::AverageJumpBuffer(SurfaceBuffer const &child_buffer,
                                        SurfaceBuffer &parent_buffer,
                                        nid_t const child_id) {

  // compute start index of second child in parent. 0 if dimension is not
  // considered
  static constexpr int x_index = int(CC::ICX()) / 2;
  static constexpr int y_index =
      CC::DIM() != Dimension::One ? int(CC::ICY()) / 2 : 0;
  static constexpr int z_index =
      CC::DIM() == Dimension::Three ? int(CC::ICZ()) / 2 : 0;
  ...
  for (BoundaryLocation const location : CC::ANBS()) {
    ...
    int const index_one_start =
        first_index_look_up_table[PositionOfNodeAmongSiblings(child_id)]
                                 [LTI(location)];
    int const index_two_start =
        second_index_look_up_table[PositionOfNodeAmongSiblings(child_id)]
                                  [LTI(location)];
    ...
    if (index_one_start >= 0 && index_two_start >= 0) {
      for (unsigned int e = 0; e < MF::ANOE(); ++e) {
        ...
        for (unsigned int i = index_one_start;
             i < index_one_start + std::ceil(double(CC::ICY()) / 2); ++i) {
          j_child = 0;
          for (unsigned int j = index_two_start;
               j < index_two_start + std::ceil(double(CC::ICZ()) / 2); ++j) {
            if constexpr (CC::DIM() == Dimension::One) {
              parent_values[e][i][j] += child_values[e][i_child][j_child];
            }
            if constexpr (CC::DIM() == Dimension::Two) {
              parent_values[e][i][j] +=
                  (child_values[e][i_child][j_child] +
            ...
            if constexpr (CC::DIM() == Dimension::Three) {
              parent_values[e][i][j] +=
                  ((child_values[e][i_child][j_child] +
                    child_values[e][i_child + 1][j_child + 1]) +
                   (child_values[e][i_child + 1][j_child] +
                    child_values[e][i_child][j_child + 1])) *
                  0.25;
            }
            j_child += 2;
          } // j
...
/**
 * @brief Fifth-order prediction according to \cite Kaiser2019. Applied only to
 * cells according to the given range.
 ...
 * @param x_start,y_start,z_start Start index in X/Y/Z-direction. Gives the
 * first cell number that should be filled in the child.
 * @param x_count,y_count,z_count Number of cells in X/Y/Z-direction that should
 * be filled in the child!
 * @note No sanity checks on start or count values are done. Values in the child
 * buffer are overriden.
 */
void Multiresolution::Prediction(
    double const (&parent_values)[CC::TCX()][CC::TCY()][CC::TCZ()],
    double (&child_values)[CC::TCX()][CC::TCY()][CC::TCZ()],
    nid_t const child_id, unsigned int const x_start,
    unsigned int const x_count, unsigned int const y_start,
    unsigned int const y_count, unsigned int const z_start,
    unsigned int const z_count) {
#ifndef PERFORMANCE
  if (x_start % 2 != 0 || y_start % 2 != 0 || z_start % 2 != 0) {
    throw std::logic_error(
        "Fatal Error in Prediction starting indices not multiple of 2");
  }
#endif
  ...
  // integer divisions are automatically rounded to floor. This is intended in
  // the expressions below.
  unsigned int const parent_x_start = (x_start / 2) + x_offset;
  unsigned int const parent_y_start =
      CC::DIM() != Dimension::One ? ((y_start / 2) + y_offset) : 0;
  unsigned int const parent_z_start =
      CC::DIM() == Dimension::Three ? ((z_start / 2) + z_offset) : 0;
  unsigned int const parent_x_end = parent_x_start + (x_count / 2);
  unsigned int const parent_y_end =
      CC::DIM() != Dimension::One ? (parent_y_start + (y_count / 2)) : 1;
  unsigned int const parent_z_end =
      CC::DIM() == Dimension::Three ? (parent_z_start + (z_count / 2)) : 1;
  ...
  // We traverse the complete parent. Loop is offsetted because stencil reaches
  // further (fifth-order interpolation)
  /**
...
/**
 * @brief Implementation of Meta function for L-infinity norm. See meta
 * function.
 */
template <>
RemeshIdentifier Multiresolution::ChildNeedsRemeshing<Norm::Linfinity>(
    Block const &parent, Block const &child, nid_t const child_id) const {

  double predicted_values[CC::TCX()][CC::TCY()][CC::TCZ()];
  double max_detail = 0.0;

  for (Equation const eq : MF::EWA()) {
    double const(&exact_values)[CC::TCX()][CC::TCY()][CC::TCZ()] =
        child.GetRightHandSideBuffer(eq);
    Multiresolution::Prediction(parent.GetRightHandSideBuffer(eq),
                                predicted_values, child_id);
    for (unsigned int i = 0; i < CC::TCX(); ++i) {
      for (unsigned int j = 0; j < CC::TCY(); ++j) {
        for (unsigned int k = 0; k < CC::TCZ(); ++k) {
          max_detail =
              std::max(max_detail, std::abs(exact_values[i][j][k] -
                                            predicted_values[i][j][k]) /
                                       std::abs(exact_values[i][j][k]));
        } // Loop : k
      }   // Loop : j
    }     // Loop : i
  }       // Loop : Equation
...
/**
 * @brief Implementation of meta function for L-one norm in three dimensions.
 * See meta function.
 */
template <>
RemeshIdentifier Multiresolution::ChildNeedsRemeshing<Norm::Lone>(
    Block const &parent, Block const &child, nid_t const child_id) const {
  ...
  for (Equation const eq : MF::EWA()) {
    ...
    for (unsigned int i = 0; i < CC::TCX(); ++i) {
      for (unsigned int j = 0; j < CC::TCY(); ++j) {
        for (unsigned int k = 0; k < CC::TCZ(); ++k) {
          error_norm +=
              std::abs(exact_values[i][j][k] - predicted_values[i][j][k]) /
              std::abs(exact_values[i][j][k]);
        }
      }
    }
    max_detail =
        std::max(max_detail,
                 error_norm * one_number_of_cells); // We save the maximum
                                                    // values over the eq-loops.
  }                                                 // Loop : eq
...
/**
 * @brief Implementation of meta function for L-two norm in three dimensions.
 * See meta function.
 */
template <>
RemeshIdentifier Multiresolution::ChildNeedsRemeshing<Norm::Ltwo>(
    Block const &parent, Block const &child, nid_t const child_id) const {
  ...
  for (Equation const eq : MF::EWA()) {
    ...
    for (unsigned int i = 0; i < CC::TCX(); ++i) {
      for (unsigned int j = 0; j < CC::TCY(); ++j) {
        for (unsigned int k = 0; k < CC::TCZ(); ++k) {
          double const temp =
              (exact_values[i][j][k] - predicted_values[i][j][k]) /
              exact_values[i][j][k];
    ...
    max_detail =
        std::max(max_detail, std::sqrt(error_norm) * one_number_of_cells);
  } // loop : eq
...
/**
 * @brief Gives the remeshing decision for the given (relevant) detail and the
 ...
 */
RemeshIdentifier
Multiresolution::RemeshingDecision(double const detail,
                                   unsigned int const level) const {

  /* The 32 pops out of \cite Harten 1995. = 2^(p+1). With p being the order of
   * the prediction minus 1. Our prediction is of fifth order. If the prediction
   * order is changed this function needs to be changed. As this seems an
   * unlikely development, we keep it hard-coded.
   */
 ...
 * @param parent_tags The parent's interface tags buffer to receive the
 * "averaged" values.
 * @param child_id The id of the child.
 * @note Not a real averaging. Only importance is that the cut cell markers are
 * correctly propagated. Therefore a multiplication is (mis-)used.
 */
void Multiresolution::PropagateCutCellTagsFromChildIntoParent(
  ...
  constexpr unsigned int j_child_start =
      CC::DIM() != Dimension::One ? CC::FICY() : 0;
  constexpr unsigned int k_child_start =
  ...
  for (unsigned int i = x_start; i < x_end; ++i) {
    ...
    for (unsigned int j = y_start; j < y_end; ++j) {
      ...
      for (unsigned int k = z_start; k < z_end; ++k) {
        // ATTENTION: in the following, we multiply bools whith each other. This
        // is fine as they are implicitely casted to int.
        overflow_safe_buffer =
            (std::abs(child_tags[i_child][j_child][k_child]) >
             ITTI(IT::NewCutCell)) *
            (std::abs(child_tags[i_child + 1][j_child][k_child]) >
             ITTI(IT::NewCutCell))
#if DIMENSION == 1
#elif DIMENSION == 2
            * (std::abs(child_tags[i_child][j_child + 1][k_child]) >
               ITTI(IT::NewCutCell)) *
            (std::abs(child_tags[i_child + 1][j_child + 1][k_child]) >
             ITTI(IT::NewCutCell))
#else
            * (std::abs(child_tags[i_child][j_child + 1][k_child]) >
               ITTI(IT::NewCutCell)) *
            (std::abs(child_tags[i_child + 1][j_child + 1][k_child]) >
             ITTI(IT::NewCutCell)) *
            (std::abs(child_tags[i_child][j_child][k_child + 1]) >
             ITTI(IT::NewCutCell)) *
            (std::abs(child_tags[i_child + 1][j_child][k_child + 1]) >
             ITTI(IT::NewCutCell)) *
            (std::abs(child_tags[i_child][j_child + 1][k_child + 1]) >
             ITTI(IT::NewCutCell)) *
            (std::abs(child_tags[i_child + 1][j_child + 1][k_child + 1]) >
             ITTI(IT::NewCutCell))
#endif
            ;
        // A cut cell is shown here as a ZERO, as we compare it to be larger
        // than 1. Mixed signs in the considered child cells are only possible
        // if there is also a ZERO, hence overflow_safe_buffer would be ZERO as
        // well. It is therefore safe to use both the sign of
        // overflow_safe_buffer and an arbitrary child cell to determine the
        // sign (or ZERO) of the parent value.
        parent_tags[i][j][k] =
            static_cast<std::int8_t>(Signum(overflow_safe_buffer)) *
            Signum(child_tags[i_child][j_child][k_child]) * ITTI(IT::BulkPhase);
        k_child += 2;
      }
      j_child += 2;
    }
...
/**
 * @brief Populates the parent's interface tags with the given uniform child
 * value.
 * @param child_tag The uniform interface tag.
 * @param parent_tags The parent's tag array to be updated based on the child's
 * tag.
 * @param child_id The id of the child.
 */
void Multiresolution::PropagateUniformTagsFromChildIntoParent(
    std::int8_t const child_tag,
    std::int8_t (&parent_tags)[CC::TCX()][CC::TCY()][CC::TCZ()],
    nid_t const child_id) {
