...
namespace Initialization {
   ...
   Material InitializeMaterial( unsigned int const material_index,
      ...
      // create eos
      std::unique_ptr<EquationOfState const> eos( InitializeEquationOfState( eos_name, eos_data, unit_handler ) );
      ...
      if constexpr( CC::ViscosityIsActive() ) {
         if( shear_viscosity_model_name != MaterialPropertyModelName::NotUsed ) {
            ...
            shear_viscosity_model = InitializeShearViscosityModel( shear_viscosity_model_name, shear_viscosity_model_data, unit_handler );
         } else {
      ...
      if constexpr( CC::HeatConductionActive() ) {
         if( thermal_conductivity_model_name != MaterialPropertyModelName::NotUsed ) {
            // create model (logs data itself)
            logger.LogMessage( tmp_string );
            thermal_conductivity_model = InitializeThermalConductivityModel( thermal_conductivity_model_name, thermal_conductivity_model_data, unit_handler );
         } else {
