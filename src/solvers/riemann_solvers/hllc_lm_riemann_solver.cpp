 ...
 */
HllcLMRiemannSolver::HllcLMRiemannSolver( MaterialManager const& material_manager, EigenDecomposition const& eigendecomposition_calculator ) : RiemannSolver( material_manager, eigendecomposition_calculator ) {
   /* Empty besides initializer list*/
}

/**
 * @brief Solving the right hand side of the underlying system of equations. Using local characteristic decomposition
 *        in combination with spatial reconstruction of cell averaged values (finite volume approach) and flux determination by central HLLC-LM procedure.
 *        See base class.
 */
void HllcLMRiemannSolver::UpdateImplementation( std::pair<MaterialName const, Block> const& mat_block, double const cell_size,
...
/**
 * @brief Computes the cell face fluxes with the set stencil using local characteristic decomposition & central HLLC-LM procedure.
...
void HllcLMRiemannSolver::ComputeFluxes( std::pair<MaterialName const, Block> const& mat_block,
   ...
   for( unsigned int i = x_start; i <= x_end; ++i ) {
      for( unsigned int j = y_start; j <= y_end; ++j ) {
         for( unsigned int k = z_start; k <= z_end; ++k ) {
            ...
            auto const [wave_speed_left, wave_speed_right] = CalculateSignalSpeed( state_face_left[ETI( Equation::Mass )], state_face_right[ETI( Equation::Mass )],
                                                                                   velocity_left, velocity_right,
                                                                                   pressure_left, pressure_right,
                                                                                   speed_of_sound_left, speed_of_sound_right,
                                                                                   gamma );

            double const wave_speed_contact = ( ( pressure_right - pressure_left ) + ( state_face_left[principal_momentum_index] * ( wave_speed_left - velocity_left ) - state_face_right[principal_momentum_index] * ( wave_speed_right - velocity_right ) ) ) / ( state_face_left[ETI( Equation::Mass )] * ( wave_speed_left - velocity_left ) - state_face_right[ETI( Equation::Mass )] * ( wave_speed_right - velocity_right ) );

            double const chi_star_left  = ( wave_speed_left - velocity_left ) / ( wave_speed_left - wave_speed_contact );
            double const chi_star_right = ( wave_speed_right - velocity_right ) / ( wave_speed_right - wave_speed_contact );
            ...
            double const Ma         = std::min( Ma_limit, std::max( std::abs( velocity_left / speed_of_sound_left ), std::abs( velocity_right / speed_of_sound_right ) ) );
            double const activation = sin( ( Ma / Ma_limit ) * M_PI_2 );
            ...
            for( unsigned int n = 0; n < MF::ANOE(); ++n ) {
               ...
               if( wave_speed_left >= 0.0 ) {
                  fluxes[n][i_index][j_index][k_index] += flux_left[n];
               } else if( wave_speed_right <= 0.0 ) {
                  fluxes[n][i_index][j_index][k_index] += flux_right[n];
               } else {
                  fluxes[n][i_index][j_index][k_index] += 0.5 * ( ( flux_left[n] + flux_right[n] ) - ( ( ( activation * wave_speed_right ) * ( state_face_right[n] - q_star_right[n] ) + ( activation * wave_speed_left ) * ( state_face_left[n] - q_star_left[n] ) ) + std::abs( wave_speed_contact ) * ( q_star_right[n] - q_star_left[n] ) ) );
               }
            }
         }// k
      }   // j
