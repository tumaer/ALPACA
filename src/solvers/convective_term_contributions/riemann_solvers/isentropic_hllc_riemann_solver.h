...
/**
 * @brief Discretization of the Riemann solver using the HLLC procedure derived from \cite Toro2009, chapter 10.4 for an isentropic case.
 */
class IsentropicHllcRiemannSolver : public RiemannSolver<IsentropicHllcRiemannSolver> {
   ...
   std::array<double, MF::ANOE()> SolveRiemannProblemImplementation( MaterialName const material,
      ...
      for( unsigned int n = 0; n < MF::ANOE(); ++n ) {
         ...
         fluxes[n]                    = ( 0.5 * ( 1.0 + Signum( wave_speed_contact ) ) * flux_star_left + 0.5 * ( 1.0 - Signum( wave_speed_contact ) ) * flux_star_right );
      }
...
public:
   ...
   explicit IsentropicHllcRiemannSolver( MaterialManager const& material_manager, EigenDecomposition const& eigendecomposition_calculator ) : RiemannSolver( material_manager, eigendecomposition_calculator ) {}
   ~IsentropicHllcRiemannSolver()                                    = default;
   IsentropicHllcRiemannSolver( IsentropicHllcRiemannSolver const& ) = delete;
   IsentropicHllcRiemannSolver& operator=( IsentropicHllcRiemannSolver const& ) = delete;
   IsentropicHllcRiemannSolver& operator=( IsentropicHllcRiemannSolver&& ) = delete;
};
