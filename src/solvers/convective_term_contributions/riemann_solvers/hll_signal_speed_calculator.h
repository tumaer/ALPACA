...
inline std::pair<double, double> CalculateSignalSpeed( double const density_left, double const density_right,
   ...
   if constexpr( FiniteVolumeSettings::signal_speed_selection == SignalSpeed::Arithmetic ) {
   ...
   if constexpr( FiniteVolumeSettings::signal_speed_selection == SignalSpeed::Davis ) {
   ...
   if constexpr( FiniteVolumeSettings::signal_speed_selection == SignalSpeed::Einfeldt ) {
      ...
      double const speed_of_sound_einfeldt = std::sqrt( ( ( speed_of_sound_left * speed_of_sound_left * sqrt_density_left ) + ( speed_of_sound_right * speed_of_sound_right * sqrt_density_right ) ) * rho_div + ( 0.5 * sqrt_density_left * sqrt_density_right * rho_div * rho_div ) * ( velocity_right - velocity_left ) * ( velocity_right - velocity_left ) );
   ...
   if constexpr( FiniteVolumeSettings::signal_speed_selection == SignalSpeed::Toro ) {
      double const z      = ( gamma - 1.0 ) / ( 2.0 * gamma );
      double const p_star = std::pow( ( ( speed_of_sound_left + speed_of_sound_right ) - 0.5 * ( velocity_right - velocity_left ) * ( gamma - 1.0 ) ) / ( speed_of_sound_left / std::pow( pressure_left, z ) + speed_of_sound_right / std::pow( pressure_right, z ) ), ( 1.0 / z ) );
      double q_l          = 1.0;
      double q_r          = 1.0;
