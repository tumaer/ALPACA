...
class HllcLMRiemannSolver : public RiemannSolver<HllcLMRiemannSolver> {
   ...
   /**
    * @brief Computes an approximate solution of a Riemann problem using the HLLC-LM procedure.
   ...
   std::array<double, MF::ANOE()> SolveRiemannProblemImplementation( MaterialName const material,
      ...
      auto const [wave_speed_left, wave_speed_right] = CalculateSignalSpeed( conservatives_left[ETI( Equation::Mass )], conservatives_right[ETI( Equation::Mass )],
                                                                             velocity_left, velocity_right,
                                                                             pressure_left, pressure_right,
                                                                             speed_of_sound_left, speed_of_sound_right,
                                                                             gamma );
      double const wave_speed_contact                = ( ( pressure_right - pressure_left ) + ( conservatives_left[principal_momentum_index] * ( wave_speed_left - velocity_left ) - conservatives_right[principal_momentum_index] * ( wave_speed_right - velocity_right ) ) ) / ( conservatives_left[ETI( Equation::Mass )] * ( wave_speed_left - velocity_left ) - conservatives_right[ETI( Equation::Mass )] * ( wave_speed_right - velocity_right ) );
      ...
      for( unsigned int n = 0; n < MF::ANOE(); ++n ) {
         if( wave_speed_left >= 0.0 ) {
            ...
            fluxes[n] = 0.5 * ( ( flux_left[n] + flux_right[n] ) - ( ( ( activation * wave_speed_right ) * ( conservatives_right[n] - q_star_right[n] ) + ( activation * wave_speed_left ) * ( conservatives_left[n] - q_star_left[n] ) ) + std::abs( wave_speed_contact ) * ( q_star_right[n] - q_star_left[n] ) ) );
         }
      }
      return fluxes;
   }
...
public:
   ...
   explicit HllcLMRiemannSolver( MaterialManager const& material_manager, EigenDecomposition const& eigendecomposition_calculator ) : RiemannSolver( material_manager, eigendecomposition_calculator ) {}
