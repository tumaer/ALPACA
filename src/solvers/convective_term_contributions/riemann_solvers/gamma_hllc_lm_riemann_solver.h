...
/**
 * @brief Discretization of the Riemann solver using the HLLC-LM procedure according to \cite Fleischmann2020b with extension to gamma states according to \Saurel2009.
 */
class GammaHllcLMRiemannSolver : public RiemannSolver<GammaHllcLMRiemannSolver> {
   ...
   std::tuple<std::array<double, MF::ANOE()>, double> SolveGammaRiemannProblemImplementation( MaterialName const,
      ...
      fluxes[ETI( Equation::Gamma )] = ( sig_left * conservatives_left[ETI( Equation::Gamma )] * wave_speed_contact ) + ( sig_right * conservatives_right[ETI( Equation::Gamma )] * wave_speed_contact );
      fluxes[ETI( Equation::Pi )]    = ( sig_left * conservatives_left[ETI( Equation::Pi )] * wave_speed_contact ) + ( sig_right * conservatives_right[ETI( Equation::Pi )] * wave_speed_contact );
...
public:
   ...
   explicit GammaHllcLMRiemannSolver( MaterialManager const& material_manager, EigenDecomposition const& eigendecomposition_calculator ) : RiemannSolver( material_manager, eigendecomposition_calculator ) {}
   ~GammaHllcLMRiemannSolver()                                 = default;
   GammaHllcLMRiemannSolver( GammaHllcLMRiemannSolver const& ) = delete;
   GammaHllcLMRiemannSolver& operator=( GammaHllcLMRiemannSolver const& ) = delete;
