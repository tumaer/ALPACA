 ...
 */
CommunicationManager::CommunicationManager(TopologyManager &topology,
                                           unsigned int const maximum_level)
    :                       // Start initializer list
      CommunicationTypes(), // For allocation of the MPI Datatypes
      topology_(topology), maximum_level_(maximum_level),
      ...
      internal_multi_boundaries_(), internal_multi_boundaries_mpi_(),
      internal_boundaries_jump_(maximum_level_ + 1),
      internal_boundaries_jump_mpi_(maximum_level_ + 1),
      external_boundaries_(maximum_level_ + 1), external_multi_boundaries_(),
...
/**
 * @brief Counts the necessary amount of planes, sticks and cubes for jump
 * sends.
 * @param loc Location necessary to determine type.
 * @param jump_send_counters [0] plane, [1] stick and [2] cube count.
 */
void IncrementJumpSendCounter(BoundaryLocation const loc,
                              std::array<unsigned int, 3> &jump_send_counters) {
...
/**
 * @brief Computes and caches the relations between ranks (who is whose
 * communication partner).
 * @param level The level on which the relations shall be obtained and cached.
 */
void CommunicationManager::GenerateNeighborRelationForHaloUpdate(
  ...
  std::vector<std::tuple<nid_t, BoundaryLocation>>
      tmp_external_id_location_vector;
  tmp_neighbor_location_vector.reserve(26);
  tmp_external_id_location_vector.reserve(6);
  // Get List of Halos on this Level
  // All (globally existing) nodes are viewed by all ranks in order to have the
  // same order on all ranks, so that the tagging system works properly.
  for (auto const global_id : topology_.IdsOnLevel(level)) {
    NeighborsOfNode(global_id, tmp_neighbor_location_vector,
                    tmp_external_id_location_vector);
    if (topology_.NodeIsOnRank(global_id, my_rank_id_)) {
      external_boundaries_[level].insert(
          external_boundaries_[level].end(),
          tmp_external_id_location_vector.begin(),
          tmp_external_id_location_vector.end());
      if (level == maximum_level_ && topology_.IsNodeMultiPhase(global_id)) {
        external_multi_boundaries_.insert(
            external_multi_boundaries_.end(),
            tmp_external_id_location_vector.begin(),
            tmp_external_id_location_vector.end());
      }
    }
    for (auto const &neighbor_id_location_element :
         tmp_neighbor_location_vector) {
      ...
      if (!topology_.NodeExists(std::get<0>(neighbor_id_location_element))) {
        ...
        if (topology_.NodeIsOnRank(global_id, my_rank_id_)) {
          ...
          if (topology_.NodeIsOnRank(parent_id, my_rank_id_)) {
            ...
            internal_boundaries_jump_[level].push_back(std::make_tuple(
                global_id, std::get<1>(neighbor_id_location_element),
                InternalBoundaryType::JumpBoundaryLocal));
          } else {
            // I have the child, but not the parent
            internal_boundaries_jump_mpi_[level].push_back(std::make_tuple(
                global_id, std::get<1>(neighbor_id_location_element),
                InternalBoundaryType::JumpBoundaryMpiRecv));
          }
        } else if (topology_.NodeIsOnRank(parent_id, my_rank_id_)) {
          // I have the parent, but not the child -> I Send the needed Info
          internal_boundaries_jump_mpi_[level].push_back(std::make_tuple(
              global_id, std::get<1>(neighbor_id_location_element),
              InternalBoundaryType::JumpBoundaryMpiSend));
          IncrementJumpSendCounter(std::get<1>(neighbor_id_location_element),
                                   jump_send_count_[level]);
        }
      } else {
        // No Jump
        bool const my_node = topology_.NodeIsOnRank(global_id, my_rank_id_);
        bool const my_neighbor = topology_.NodeIsOnRank(
            std::get<0>(neighbor_id_location_element), my_rank_id_);
        if (my_node) {
          if (my_neighbor) {
            internal_boundaries_[level].push_back(std::make_tuple(
                global_id, std::get<1>(neighbor_id_location_element),
                InternalBoundaryType::NoJumpBoundaryLocal));
          } else {
            internal_boundaries_mpi_[level].push_back(std::make_tuple(
                global_id, std::get<1>(neighbor_id_location_element),
                InternalBoundaryType::NoJumpBoundaryMpiRecv));
          }
        } else {
          if (my_neighbor) {
            internal_boundaries_mpi_[level].push_back(std::make_tuple(
                std::get<0>(neighbor_id_location_element),
                OppositeDirection(std::get<1>(neighbor_id_location_element)),
                InternalBoundaryType::NoJumpBoundaryMpiSend));
          }
        }
        if (level == maximum_level_ && topology_.IsNodeMultiPhase(global_id) &&
            topology_.IsNodeMultiPhase(
                std::get<0>(neighbor_id_location_element))) {
          if (my_node) {
            if (my_neighbor) {
              internal_multi_boundaries_.push_back(std::make_tuple(
                  global_id, std::get<1>(neighbor_id_location_element),
                  InternalBoundaryType::NoJumpBoundaryLocal));
            } else {
              internal_multi_boundaries_mpi_.push_back(std::make_tuple(
                  global_id, std::get<1>(neighbor_id_location_element),
                  InternalBoundaryType::NoJumpBoundaryMpiRecv));
            }
          } else {
 ...
 * e.g. std::bitset<26> (CC::ANBL()[LTI(EAST)] gives all halo positions
 * contained in east natural boundary side, e.g. east-south-bottom. The bitset
 * fills the tables with the configuration below with 0 or 1. wsb, wst, wnb,
 * wnt, esb, est, enb, ent, sw, se, nw, ne, tw, te, bw, be, ts, tn, bs, bn, b,
 * t, s, n, w, e. returns an unsigned int that has all bits set for a specific
...
/**
 * @brief Finds the neighbors of a specific node and sorts them into the array.
 * Domain Boundaries are inserted only as natural (e,w,n,s,t,b), internals are
 * inserted per direction e.g diagonal wnb.
 * @param global_id global node's id.
 * @param nodes_internal_boundaries output array for internal boundaries, all
 * HaloBoundarySides are included that are not part of externals BC.
 * @param external_boundaries all external boundaries are included as natural
 * Boundary Side e.g. east, west...
 */
void CommunicationManager::NeighborsOfNode(
    nid_t const global_id,
    std::vector<std::tuple<nid_t, BoundaryLocation>> &nodes_internal_boundaries,
  ...
  for (BoundaryLocation loc : CC::ANBS()) {
    if (topology_.IsExternalTopologyBoundary(loc, global_id)) {
      sides |=
          std::bitset<26>(bitsets_for_natural_boundary_locations[LTI(loc)]);
...
/**
 * @brief Tells the communication manager that there was a change in the
 * topology and it needs to generate the material boundaries from scratch.
 */
 ...
 */
int CommunicationManager::Send(void const *buffer, int const count,
                               MPI_Datatype const datatype,
                               int const destination_rank,
                               std::vector<MPI_Request> &requests) {
  int const tag = TagForRank(destination_rank);
  requests.push_back(MPI_Request());
  int const status = MPI_Isend(buffer, count, datatype, destination_rank, tag,
 ...
 * @note Must not be called in single-core mode (will hang).
 */
int CommunicationManager::Recv(void *buffer, int const count,
                               MPI_Datatype const datatype,
                               int const source_rank,
                               std::vector<MPI_Request> &requests) {
  int const tag = TagForRank(source_rank);
  requests.push_back(MPI_Request());
  int const status = MPI_Irecv(buffer, count, datatype, source_rank, tag,
                               MPI_COMM_WORLD, &requests.back());
#ifndef PERFORMANCE
  if (status != MPI_SUCCESS) {
...
/**
 * @brief Returns a tag, that can be used for one communication with another
 * rank. TagForRank must be called in the same order on all participating ranks.
 * If replacing the communication with MPI_Send/MPI_Recv is valid, it's OK.
...
/**
 * @brief Resets the tags used for communication between ranks to reduce tag
 * overflows over the mpi tag upper bound.
 */
...
/**
 * @brief Gives a reference to the list for a given level holding all internal
 * jump boundary relations that require mpi communication.
...
CommunicationManager::InternalBoundariesJumpMpi(
...
/**
 * @brief Gives a reference to the list for a given level holding all internal
 * jump boundary relations that do not require mpi communication.
 * @param level Level for which the list should be returned.
 * @return List with relations for all internal no-mpi jump boundaries.
 */
std::vector<std::tuple<nid_t, BoundaryLocation, InternalBoundaryType>> const &
CommunicationManager::InternalBoundariesJump(unsigned int const level) const {
  return internal_boundaries_jump_[level];
}

/**
 * @brief Gives a reference to the list for a given level holding all internal
 * non-jump boundary relations that require mpi communication.
 * @param level Level for which the list should be returned.
 * @return List with relations for all internal mpi non-jump boundaries.
 */
std::vector<std::tuple<nid_t, BoundaryLocation, InternalBoundaryType>> const &
CommunicationManager::InternalBoundariesMpi(unsigned int const level) const {
  return internal_boundaries_mpi_[level];
}

/**
 * @brief Gives a reference to the list for a given level holding all internal
 * non-jump boundary relations that do not require mpi communication.
 * @param level Level for which the list should be returned.
 * @return List with relations for all internal no-mpi non-jump boundaries.
 */
std::vector<std::tuple<nid_t, BoundaryLocation, InternalBoundaryType>> const &
CommunicationManager::InternalBoundaries(unsigned int const level) const {
  return internal_boundaries_[level];
}

/**
 * @brief Gives a reference to the list of multi-material nodes holding all
 * internal boundary relations that require mpi communication.
...
/**
 * @brief Gives a reference to the list of multi-material nodes holding all
 * internal boundary relations that do not require mpi communication.
 * @return List with relations for all internal no-mpi multi-material
 * boundaries.
 */
std::vector<std::tuple<nid_t, BoundaryLocation, InternalBoundaryType>> const &
...
/**
 * @brief Gives a reference to the list for a given level holding all external
 * boundary relations.
 * @param level Level for which the list should be returned.
 * @return List with relations for all external boundary nodes.
 */
std::vector<std::tuple<nid_t, BoundaryLocation>> const &
CommunicationManager::ExternalBoundaries(unsigned int const level) const {
  return external_boundaries_[level];
}

/**
 * @brief Gives a reference to the list of multi-material nodes on the maximum
 * level holding all external boundary relations.
 * @return List with relations for all external boundary nodes.
 */
std::vector<std::tuple<nid_t, BoundaryLocation>> const &
CommunicationManager::ExternalMultiBoundaries() const {
...
/**
 * @brief Gives a reference to the list for a given level holding all internal
 * jump boundaries that require mpi communication.
 * @param level Level for which the list should be returned.
 * @param type Type of the exchange buffer that is used.
 * @return List with internal mpi jump boundaries.
 */
unsigned CommunicationManager::JumpSendCount(unsigned const level,
                                             ExchangeType const type) {
