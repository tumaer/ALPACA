...
/**
 * @brief The CommunicationTypes class collects all information about the
 * different types used for the MPI-communication. It specifies the sizes, start
 * indices and other information to allow specific buffer communication in one
 * single chunk.
 */
class CommunicationTypes {

  static constexpr std::array<int, 3> block_size_ = {CC::TCX(), CC::TCY(),
                                                     CC::TCZ()};
  // 26 Elements for 3 Dimensions, numbering like boundary_specification.h ->
  // BoundaryLocation
  ...
  // Averaging
  static constexpr std::array<int, 3> child_size_ = {
      {CC::PSOCICX(), CC::PSOCICY(), CC::PSOCICZ()}};
  ...
  // 26 Elements for 3 Dimensions, numbering like boundary_specification.h
  // ->BoundaryLocation 1D: East, West 2D: East, West, North, South 3D: East,
  ...
  /**
   * @brief CreateDataTypes Creates and commits MPI datatypes for in order to
   * exchange boundary subarrays and such in one go as one subarray.
   */
...
public:
  ...
  MPI_Datatype SendDatatype(BoundaryLocation const location,
                            DatatypeForMpi const datatype) const;
  MPI_Datatype RecvDatatype(BoundaryLocation const location,
                            DatatypeForMpi const datatype) const;
  ...
  MPI_Datatype AveragingSendDatatype(unsigned int const child_position,
                                     DatatypeForMpi const datatype) const;

  static constexpr std::array<int, 3>
  GetStartIndicesHaloSend(BoundaryLocation const location) {
    return start_indices_halo_send_[LTI(location)];
  }
  static constexpr std::array<int, 3>
  GetStartIndicesHaloRecv(BoundaryLocation const location) {
    return start_indices_halo_recv_[LTI(location)];
  }
  static constexpr std::array<int, 3>
  GetHaloSize(BoundaryLocation const location) {
    return halo_size_[LTI(location)];
  }
};

/**
 * @brief Gives the total number values need to be transferred per conservative
 * variable (+buffer copies), which need to be transferred if the content of a
 * whole block is to be sent from one rank to another.
 * @return The size of a full block (internal + halos).
 */
constexpr int FullBlockSendingSize() {
  return CC::TCX() * CC::TCY() * CC::TCZ();
}

/**
 * @brief Gives the total number values need to be transferred per conservative
 * variable (+buffer copies), if the content of all internal cells of a whole
 * block is to be sent from one rank to another.
 * @return The size of the internal block.
 */
constexpr int DomainBlockSendingSize() {
  return CC::ICX() * CC::ICY() * CC::ICZ();
}

/**
 * @brief Gives the total number of values to be transferred if the jump flux
 * buffers of all conservative variables are to be sent form one rank to another
 * at once.
 * @return The size of jump flux buffer.
 */
constexpr int JumpBufferSendingSize() {
  return MF::ANOE() * CC::ICY() * CC::ICZ();
}
