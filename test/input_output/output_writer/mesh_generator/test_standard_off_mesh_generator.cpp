...
/***********************************/
SCENARIO( "Standard mesh generator without mpi-filtering: Dimension of global vertex coordinates vector is correctly computed", "[1rank],[2rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      std::unique_ptr<MeshGenerator const> mesh_generator = std::make_unique<StandardMpiMeshGenerator const>( topology, tree, 1.0, false );
      ...
      WHEN( "The node in the topology is refined" ) {
         ...
         THEN( "The global vertex coordinates dimension vector has two entries with the first being equal the number of vertices time the number of leafs (8) and the second being three" ) {
            std::vector<hsize_t> const global_vertex_coordinates_dimensions = mesh_generator->GetGlobalDimensionsOfVertexCoordinates();
            REQUIRE( global_vertex_coordinates_dimensions.size() == 2 );
            REQUIRE( global_vertex_coordinates_dimensions[0] == MeshGeneratorUtilities::NumberOfInternalVerticesPerBlock() * 8 );
            REQUIRE( global_vertex_coordinates_dimensions[1] == 3 );
         }
      }
   }
...
/***********************************/
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex Coordinates local dimensions to write are correctly determined", "[1rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      WHEN( "The node in the topology is refined" ) {
         ...
         THEN( "The local vertex coordinates dimension vector has two entries with the first being equal the number of vertices time the number of leafs (8) and the second being three" ) {
            std::vector<hsize_t> const local_vertex_coordinates_dimensions = mesh_generator->GetLocalDimensionsOfVertexCoordinates();
            REQUIRE( local_vertex_coordinates_dimensions.size() == 2 );
            REQUIRE( local_vertex_coordinates_dimensions[0] == MeshGeneratorUtilities::NumberOfInternalVerticesPerBlock() * 8 );
            REQUIRE( local_vertex_coordinates_dimensions[1] == 3 );
         }
      }
   }
...
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex Coordinates local dimensions to write are correctly determined on 2 tanks", "[2rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      WHEN( "The node in the topology is refined" ) {
         TestUtilities::RefineFirstNodeInTopology( topology, true );
         REQUIRE( topology.NodeAndLeafCount() == std::pair<unsigned int, unsigned int>( 9, 8 ) );

         THEN( "The local vertex coordinates dimension vector has two entries with the first being equal the number of vertices time the number of leafs (8) and the second being three" ) {
            std::vector<hsize_t> const local_vertex_coordinates_dimensions = mesh_generator->GetLocalDimensionsOfVertexCoordinates();
            REQUIRE( local_vertex_coordinates_dimensions.size() == 2 );
            REQUIRE( local_vertex_coordinates_dimensions[0] == MeshGeneratorUtilities::NumberOfInternalVerticesPerBlock() * 4 );
            REQUIRE( local_vertex_coordinates_dimensions[1] == 3 );
         }
      }
   }
...
/* 3. Start index check            */
/***********************************/
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex Coordinates start index to write is correctly determined", "[1rank]" ) {
...
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex Coordinates start index to write is correctly determined on two ranks", "[2rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      WHEN( "The node in the topology is refined" ) {
         ...
         THEN( "The local vertex coordinates dimension vector has two entries with the first being equal the number of vertices time the number of leafs (8) and the second being three" ) {
            hsize_t const local_vertex_coordinates_start_index = mesh_generator->GetLocalVertexCoordinatesStartIndex();
            REQUIRE( local_vertex_coordinates_start_index == MpiUtilities::MyRankId() * MeshGeneratorUtilities::NumberOfInternalVerticesPerBlock() * 4 );
         }
      }
   }
}
...
/* 4. Computation check            */
/***********************************/
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex coordinates node corner points are correctly written out", "[1rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      std::array<unsigned long long int, 3> const corner_indices = { {  ( CC::ICX() + 1 ) * 3 - 3
                                                                      , ( CC::ICX() + 1 ) * ( CC::ICY() + 1 ) * 3 - 3
                                                                      , ( CC::ICX() + 1 ) * ( CC::ICY() + 1 ) * ( CC::ICZ() + 1 ) * 3 - 3 } };
      ...
      WHEN( "The node in the topology is refined" ) {
         ...
         THEN( "The coordinates vector has 3 * #of Leaves * #vertices * 8 #nodes and depending on the first coordinate of each node check corner points" ) {           
            REQUIRE( coordinates.size() == 8 * MeshGeneratorUtilities::NumberOfInternalVerticesPerBlock() * 3 );
            ...
            for( unsigned int node_start = 0; node_start < 8; node_start++ ) {
...
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex coordinates node corner points are correctly written out on two ranks", "[2rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      WHEN( "The node in the topology is refined" ) {
         ...
         THEN( "The coordinates vector has 3 * #of Leaves * #vertices * 4 #nodes and depending on the first coordinate of each node check corner points" ) {           
            REQUIRE( coordinates.size() == 4 * MeshGeneratorUtilities::NumberOfInternalVerticesPerBlock() * 3 );

            // Define the cell size (2 nodes in each direction are present, ICX() since always filled)
            double const cell_size = 1.0 / double( CC::ICX() ) / 2;

            // Loop through all node start positions and check the coordinates at the corner positions depending on that 
            for( unsigned int node_start = 0; node_start < 4; node_start++ ) {
...
/***********************************/
SCENARIO( "Standard mesh generator without mpi-filtering: Dimension of global vertex IDs vector is correctly computed", "[1rank],[2rank]" ) {
...
/***********************************/
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex IDs local dimensions to write are correctly determined", "[1rank]" ) {
...
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex IDs local dimensions to write are correctly determined on two ranks", "[2rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      WHEN( "The node in the topology is refined" ) {
         ...
         THEN( "The local vertex IDs dimension vector has two entries with the first being equal the number of vertices time the number of leafs (8) and the second being three" ) {
            std::vector<hsize_t> const local_vertex_IDs_dimensions = mesh_generator->GetLocalDimensionsOfVertexIDs();
            REQUIRE( local_vertex_IDs_dimensions.size() == 2 );
            REQUIRE( local_vertex_IDs_dimensions[0] == MeshGeneratorUtilities::NumberOfInternalCellsPerBlock() * 4 );
            REQUIRE( local_vertex_IDs_dimensions[1] == 8 );
         }
      }
   }
...
/* 3. Start index check            */
/***********************************/
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex IDs start index to write is correctly determined", "[1rank]" ) {
...
SCENARIO( "Standard mesh generator without mpi-filtering: Vertex IDs start index to write is correctly determined on two ranks", "[2rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      WHEN( "The node in the topology is refined" ) {
         ...
         THEN( "The local vertex coordinates dimension vector has two entries with the first being equal the number of vertices time the number of leafs (4) and the second being three" ) {
            hsize_t const local_vertex_IDs_start_index = mesh_generator->GetLocalVertexIDsStartIndex();
            REQUIRE( local_vertex_IDs_start_index == MpiUtilities::MyRankId() * MeshGeneratorUtilities::NumberOfInternalCellsPerBlock() * 4 );
         }
      }
   }
}
...
/********************************************************************************************************************************************/ 
SCENARIO( "Standard mesh generator without mpi-filtering: Check that vertex IDs and coordinates are computed properly in combination", "[1rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      WHEN( "The node in the topology is refined" ) {
         ...
         THEN( "The coordinates of the IDs of each cell must be in an order where first x is increased, then y and last z" ) {
            // Check the total size of both vectors
            REQUIRE( coordinates.size() == 8 * MeshGeneratorUtilities::NumberOfInternalVerticesPerBlock() * 3 );
            REQUIRE( vertex_ids.size()  == 8 * MeshGeneratorUtilities::NumberOfInternalCellsPerBlock() * 8 );
            // Check that the largest index in the vertex ids is not larger than the size of the coordinates vector
            REQUIRE( *std::max_element( vertex_ids.begin(), vertex_ids.end() ) < coordinates.size() );
...
SCENARIO( "Standard mesh generator without mpi-filtering: Check that vertex IDs and coordinates are computed properly in combination on two ranks", "[2rank]" ) {
   ...
   GIVEN( "Underlying topology with Lmax being one" ) {
      ...
      WHEN( "The node in the topology is refined" ) {
         ...
         THEN( "The coordinates of the IDs of each cell must be in an order where first x is increased, then y and last z" ) {
            // Check the total size of both vectors
            REQUIRE( coordinates.size() == 4 * MeshGeneratorUtilities::NumberOfInternalVerticesPerBlock() * 3 );
            REQUIRE( vertex_ids.size()  == 4 * MeshGeneratorUtilities::NumberOfInternalCellsPerBlock() * 8 );
            // Check that the largest index in the vertex ids is not larger than the size of the coordinates vector
            REQUIRE( *std::max_element( vertex_ids.begin(), vertex_ids.end() ) < 4 * MeshGeneratorUtilities::NumberOfInternalVerticesPerBlock() * 3 );
